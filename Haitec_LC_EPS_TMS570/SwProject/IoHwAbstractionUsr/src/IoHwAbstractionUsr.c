 ...
 * 07/31/13   2       OT        Updates for new hardware layout (removed forward path conversion)
 * 01/15/14   3       OT        Added memmap section directives
 * 04/21/14   4       OT        Updates for DMA
 * 05/28/14   5       OT        Updates for DMA 002.0
 * 02/12/15   6       OT        Added P/N step counter for slow ADC diagnostic                                   13030
 */
/**********************************************************************************************************************
...
#include "MemMap.h"
STATIC VAR(uint16, IOHWABSTRACTIONUSR_VAR) SlowADCFltAcc_Cnt_M_u16;
STATIC VAR(uint16, IOHWABSTRACTIONUSR_VAR) VrefFltAcc_Cnt_M_u16;
 ...
 *********************************************************************************************************************/
	...
	{
		SlowADCFltAcc_Cnt_M_u16 = DiagNStep_m(SlowADCFltAcc_Cnt_M_u16, k_SlowADCDiag_Cnt_str);
		if( SlowADCFltAcc_Cnt_M_u16 == 0u )
		{
			Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_PerDiag_DMAFlt, 0u, NTC_STATUS_PASSED);
		}
			...
			(Vref_Volt_T_f32 < k_VrefMin_Volt_f32) )
		{
			...
			if( DiagFailed_m(VrefFltAcc_Cnt_M_u16, k_VrefDiag_Cnt_str) == TRUE )
			{
				if( Vref_Volt_T_f32 > k_VrefMax_Volt_f32 )
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_RefSupplyVltg, 1u, NTC_STATUS_FAILED);
				}
				else
				{
					Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_RefSupplyVltg, 2u, NTC_STATUS_FAILED);
				}
			}
		}
		else
		{
			...
			{
				Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_RefSupplyVltg, 0u, NTC_STATUS_PASSED);
			}
		}
	...
	{
		SlowADCFltAcc_Cnt_M_u16 = DiagPStep_m(SlowADCFltAcc_Cnt_M_u16, k_SlowADCDiag_Cnt_str);
		if( DiagFailed_m(SlowADCFltAcc_Cnt_M_u16, k_SlowADCDiag_Cnt_str) == TRUE )
		{
			Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_PerDiag_DMAFlt, 0x20u, NTC_STATUS_FAILED);
		}
	}
