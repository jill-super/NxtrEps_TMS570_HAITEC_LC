---
title: "Cyclic Redundancy Check Library (Crc)"
description: "Vector MICROSAR Crc: role, repository files and configuration."
---

<div class="origin-badges">
  <span class="origin-badge origin-vector">Vector-provided (MICROSAR)</span>
</div>
## Role

Library of cyclic-redundancy-check calculation routines used for data protection.

:::note[Origin: Vector-provided (MICROSAR)]
This module is part of the Vector MICROSAR Basic Software delivery. Sources are proprietary to Vector Informatik GmbH and are integrated — not developed — in this repository. Configuration is generated with the DaVinci/GENy tooling for this Electronic Control Unit.
:::

## Repository files

| File |
| --- |
| `Haitec_LC_EPS_TMS570/SwProject/Source/BSW/Crc/Crc.c` |
| `Haitec_LC_EPS_TMS570/SwProject/Source/BSW/Crc/Crc.h` |

_The module directory holds 2 tracked files._

## Generated configuration (GenData)

The following generated files configure this module for the steering Electronic Control Unit:

- `Haitec_LC_EPS_TMS570/SwProject/Source/GenData/Crc_Cfg.h`

## Reference documents

- [TechnicalReference Asr Crc](../documents/technicalreference-asr-crc) — converted from `Haitec_LC_EPS_TMS570/HLDD/BSW/TechnicalReference_Asr_Crc.pdf`

